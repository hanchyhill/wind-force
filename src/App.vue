  ...
  <div id="app">
    <div class="show-control" v-show="showControl">
      <Row>
        ...
        <Col span="3">
          <Button type="primary" icon="ios-search" @click="searchData">搜索模式</Button>
        </Col>
      </Row>
      <Row>
        <Col span="8">
...
export default {
      ...
      axios.get(urlGetHourlyFc)
        .then(res => {
          ...
          if (data.DATA.length == 0) {
            ...
            series = series.map(data => data.filter(v=> v[1] > 0 && v[1] < 25));// 只保留24小时预报
  ...
  computed: {
    ...
    tableData() {
      ...
      let data = this.v10m.map((v, i) => {
        ...
        let colo = {
          ...
          wg10m: Number.parseInt(waveFit.Gust)<1?1:waveFit.Gust,
          fTime,
          iknots,
    ...
    /**模式与预报时间的差值*/
    hourShift(){
      const modelInitTime = moment(this.modelInitDate + this.modelFcHour, "YYYY-MM-DDHH:mm:ss");
      const fcInitTime = moment(this.initTime + this.fcHour, "YYYY-MM-DDHH:mm:ss");
